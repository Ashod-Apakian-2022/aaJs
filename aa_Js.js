...
/*-----------------------------------------------------------------------*/
 ...
 function guiRetinaSet (handle,x,y,w,h,retina)
 {
 if(1&&aa_profiler.is_started&&aa_profile_group_gui) { aaProfilerHit(arguments.callee.name); aaProfilerHit(arguments.callee.name+"<-"+arguments.callee.caller.name);  }
 var obj,ratio;
 if((obj=handleCheck(gui_obj.handef,handle))==null) { return false; }
...
// aa.debugLogger(5,aa.debugFunctionName());
 ...
 if(obj.state==1)
  ...
  obj.val=el;
  if(obj.val>=1.0)  {  obj.val=1.0;   obj.state=0;   }
 ...
 switch(index)
  ...
  case 3:  return(guiRgbaString(0,255,0,opa));      // green
  case 4:  return(guiRgbaString(0,200,255,opa));    //
  case 5:  return(guiRgbaString(0,255,255,opa));    // cyan
  case 6:  return(guiRgbaString(255,0,255,opa));    // purple
  case 7:  return(guiRgbaString(255,255,0,opa));    // yellow
 ...
 function guiSpotAdd (handle,sid,x,y,w,h,uv1,uv2)
 {
 if(1&&aa_profiler.is_started&&aa_profile_group_gui) { aaProfilerHit(arguments.callee.name); aaProfilerHit(arguments.callee.name+"<-"+arguments.callee.caller.name);  }
 var grp,spt,i;
 if((grp=aa.guiGroupGet(handle))==null) { return false; }
 ...
 for(i=0;i<grp.vars.spot.slots;i++)
  ...
  //if(grp.obj.id=="b_overlay_0")
   {
   //console.log(grp.obj.id+"  "+spt.sid+"  "+(x>>0)+"  "+(y>>0)+"  "+(hx1>>0)+"  "+(hy1>>0)+" "+(hx2>>0)+" "+(hy2>>0));
   //console.log(spt);
   }
...
/*-----------------------------------------------------------------------*/
 ...
 function guiWidgetNew (cat,canvasid)
 {
 var obj,grp,disp;
 if((grp=aa.guiGroupGetById(canvasid))==null) { return null; }
 disp=aa.envDisplayGet();
 obj={};
 obj.type="widget";
 obj.guc=aa_guc++;
 obj.cat=cat;
 ...
 obj.stage=0;
 obj.grp=grp;
 obj.rect=aa.guiRectSet(0,0,1,1);
 obj.vars={};
 aa.guiWidgetFontSet(obj,500,20,"Arial");
 ...
 switch(obj.stage)
  ...
  while(1)
   ...
   while(1)
    ...
    ok=0;
    if(x>=iw) { break; }
    pen=spritePixGet(x+0,obj.oy+0,obj.pix,{r:255,g:0,b:220,a:1});   if(pen.match==false) { x++; continue; }
    pen=spritePixGet(x+1,obj.oy+0,obj.pix,{r:255,g:0,b:220,a:1});   if(pen.match==false) { x++; continue; }
    pen=spritePixGet(x+0,obj.oy+1,obj.pix,{r:255,g:0,b:220,a:1});   if(pen.match==false) { x++; continue; }
    ok=1;
    break;
    }
   ...
   minX=maxX=x;
   minY=maxY=obj.oy;
   for(z=minX;z<iw;z++)  { pen=spritePixGet(z,minY,obj.pix,{r:255,g:0,b:220,a:1});  if(pen.match==false) { break; }   maxX=z;   }
   for(z=minY;z<ih;z++)  { pen=spritePixGet(minX,z,obj.pix,{r:255,g:0,b:220,a:1});  if(pen.match==false) { break; }   maxY=z;   }
  ...
  while(off<todo)
   ...
   if(obj.pix.data[off+3]>0)
    ...
    r=r*0.65;   g=g*0.65;   b=b*0.65;   r=r>>0;   g=g>>0;   b=b>>0;   a=a>>0;
...
/*-----------------------------------------------------------------------*/
 ...
 arr[7]=aa.iface_obj.this_disp.is_landscape;
 arr[8]=aa.iface_obj.this_disp.angle;
 arr[9]=aa.iface_obj.this_disp.orient;
 return arr;
 }
...
//  dobj.dom.volume=0;
 ...
 switch(obj.stage)
  ...
  prom=navigator.mediaDevices.getUserMedia(obj.avc)
...
/*-----------------------------------------------------------------------*/
 ...
 for(i=0;i<socket_obj.handef.slots;i++)
  ...
  obj.is_closed=false;
  obj.is_error=false;
  obj.is_direct=false;//true;//false;//true;
  obj.code=0;
  ...
  obj.socket.onopen=function(event)  {   obj.is_open=true;    }
  obj.socket.onclose=function(event) {   obj.is_closed=true; obj.code=event.code; }
  obj.socket.onerror=function(event) {   obj.is_error=true;  obj.code=event.code; }
...
/*-----------------------------------------------------------------------*/
 ...
 //obj.stage=stage;
 //console.log("rtc promise stage="+stage);
 //console.log(obj.vars.pc.localDescription);
 switch(stage)
  {
 ...
 obj.promise_status=aa.promiseStatus(obj.promise_handle);
 if(obj.promise_status.state!=1) { aa.debugAlert("ps.state="+obj.promise_status.state); return false; }
 ...
 switch(name)
  ...
  //console.log(rtc.user_data+"  "+name+"  "+pc.signalingState+"  @@@@@@@@@@");
...
//  console.log(rtc.user_data+"  "+name+"  "+pc.iceGatheringState);
  break;

  case "oniceconnectionstatechange":
  //aa.debugLogger(25,rtc.user_data+"  "+name+"  "+pc.iceConnectionState);
//console.log(rtc.user_data+"  "+name+"  "+pc.iceConnectionState);
  break;
  ...
  //if(event.candidate==null) { console.log("EV null"); }
  //if(event.candidate=="")   { console.log("EV empty"); }
  ...
  case "ondatachannel":
  ///console.log("*ondch"+rtc.vars.user_data+" "+name+" "+event.channel.readyState+" "+event.channel.label);
 ...
 switch(event.type)
  ...
  case "open":
  rtc.data_open_count++;
  console.log(rtc.user_data+"         onData "+event.type+"  "+event.currentTarget.label);
  break;
...
/// if(main_obj.state.worker_array.length>=2) { console.log(main_obj.state.worker_array.length); alert(); }
 ...
 main_obj.state.thread_id=window.setTimeout(function()
  ...
  if(main_obj.state.is_running==true)
   {
   main_obj.initial_dmm=aa.debugMemoryUsage();
   mainRun();
   }
  else
   {
...
/*-----------------------------------------------------------------------*/
 ...
 switch(obj.this_stage)
  ...
  obj.import_obj=
   ...
   env:
    ...
    ext_consoleFog:
    function(arg,len)     { console.log(len+" "+arg); },
    ...
    ext_atan2:
    function(x,y)         { return(Math.atan2(x,y)); },
    ...
    ext_graReset:
    function()            {  return(aa.mainWasmGraReset(obj));          } ,

    ext_graAdd:
    function(type,len)    {  return(aa.mainWasmGraAdd(obj,type,len));   } ,

    ext_graGet:
    function(index)       {  return(aa.mainWasmGraGet(obj,index));   } ,

    ext_graTextSet:
    function(index,str)   {  return(aa.mainWasmGraTextSet(obj,index,str));   } ,

    ext_graTextGet:
    function(ptr,len)     {  return(aa.mainWasmGraTextGet(obj,ptr,len));   } ,
 ...
 switch(type)
  ...
  if(type==1) { type="s8";  }
  nam="Int8Array";
  gra=new Int8Array(obj.memory.buffer,off,len);
  ...
  if(type==2) { type="u8";  }
  nam="Uint8Array";
  gra=new Uint8Array(obj.memory.buffer,off,len);
  ...
  if(type==3) { type="s16";  }
  nam="Int16Array";
  gra=new Int16Array(obj.memory.buffer,off,len);
  ...
  if(type==3) { type="u16";  }
  nam="Uint16Array";
  gra=new Uint16Array(obj.memory.buffer,off,len);
  add=(len*Uint16Array.BYTES_PER_ELEMENT);
  break;
  ...
  if(type==5) { type="s32";  }
  nam="Int32Array";
  gra=new Int32Array(obj.memory.buffer,off,len);
  add=(len*Int32Array.BYTES_PER_ELEMENT);
  break;
  ...
  if(type==6) { type="u32";  }
  nam="Uint32Array";
  gra=new Uint32Array(obj.memory.buffer,off,len);
  ...
  if(type==7) { type="f32";  }
  nam="Float32Array";
  gra=new Float32Array(obj.memory.buffer,off,len);
  add=(len*Float32Array.BYTES_PER_ELEMENT);
  break;
  ...
  if(type==8) { type="f64";  }
  nam="Float64Array";
  gra=new Float64Array(obj.memory.buffer,off,len);
  ...
  if(type==9) { type="s64";  }
  nam="BigInt64Array";
  gra=new BigInt64Array(obj.memory.buffer,off,len);
  add=(len*BigInt64Array.BYTES_PER_ELEMENT);
  break;
  ...
  if(type==10) { type="u64";  }
  nam="BigUint64Array";
  gra=new BigUint64Array(obj.memory.buffer,off,len);
  ...
  gra=new Uint8ClampedArray(obj.memory.buffer,off,len);
...
//         2=include only pathed
 ...
 {
 var raylen,index,obj,txt,str,all,perc,band,remain,num,sio,count;
